...
- Consume values only from the `@repo/tokens` package — reference the CSS custom properties (`var(--…)`) or the exported TS tokens. Never hardcode colors, spacing, radii, type sizes, or shadows.
...
- Do NOT modify any token value to fix it. Wait for my explicit approval before changing anything in `@repo/tokens`.