...
import { indicatorFromSlug } from '@/shared/model/ranking-indicators'
...
export function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith('/rankings/')) {
    return segment && indicatorFromSlug(segment) ? NextResponse.next() : notFound(request)
  }
...
export const config = {
  matcher: ['/countries/:id', '/region/:region', '/income/:level', '/rankings/:indicator']
}
